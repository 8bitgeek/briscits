...
void notify_heap_alloc_failed(size_t size)
{
	...
	* @return void 
	*/
	void brisc_heap_init(void* heap_base, void* heap_end)
	{
		...
		#if BRISC_HEAP_MUTEX
			brisc_mutex_set_flags(&malloc_mutex,BRISC_MUTEX_F_RECURSIVE | BRISC_MUTEX_F_NOYIELD);
	    #endif

		BRISC_MALLOC_LOCK();
		
		++heap_count; heap_num=heap_count-1;
		while(heap_count>BRISC_NUMHEAPS); /* FIXME - trap too-many-heaps error */

		/** Initialize the parameters based on the linkage map */
		memset(HEAP_STATE(heap_num),0,sizeof(brisc_heap_state_t));
	    ...
	    #if defined(BRISC_ZERO_FILL_HEAP)
	...
	*/
	extern int32_t brisc_heap_block_size()
	{
	...
	*/
	...
	{
		int32_t rc = brisc_heap_blocks_allocated()*HEAP_BLOCK_SIZE;
		return rc;
	}
	...
	extern int32_t brisc_heap_bytes_free()
	{
		...
		rc += (heap_blocks-brisc_heap_blocks_allocated())*HEAP_BLOCK_SIZE;
		BRISC_MALLOC_UNLOCK();
		return rc;
	}
	 ...
	 */
	static bool get_bitmap_bit(brisc_heap_state_t* heap_state,int32_t block,uint32_t map[])
	{
	 ...
	 */
	static void set_bitmap_bit(brisc_heap_state_t* heap_state,int32_t block,uint32_t map[],bool state)
	{
		if ( valid(heap_state,block) )
		{
	 ...
	 */
	static bool is_free_sequence(brisc_heap_state_t* heap_state, int32_t block, int32_t blocks)
	{
		register int32_t n;
	 ...
	 */
	static int32_t block_range(brisc_heap_state_t* heap_state, int32_t* blocks)
	{
		int32_t rc = 0;
	 ...
	 */
	static int32_t blocks_used(brisc_heap_state_t* heap_state,int32_t block)
	{
		int32_t used=0;
		if(valid(heap_state,block))
		{
	 ...
	 */
	static int32_t from_pointer(brisc_heap_state_t* heap_state,void* pointer)
	{
		if (pointer!=NULL)
		{
	...
	*/
	 ...
	 */
	static void* to_pointer(brisc_heap_state_t* heap_state,int32_t block)
	{
		void* pointer=NULL;
		if (valid(heap_state,block))
		{
	 ...
	 */
	static bool deallocate(brisc_heap_state_t* heap_state, register int32_t block, register int32_t blocks)
	{
		if ( valid(heap_state,block) )
		{
	 ...
	 */
	static bool can_extend(brisc_heap_state_t* heap_state, int32_t block, int32_t used, int32_t blocks)
	{
		register int32_t n;
		bool rc=false;
		if ( valid(heap_state,block) )
	 ...
	 */
	static bool extend(brisc_heap_state_t* heap_state, int32_t block, int32_t used, int32_t blocks)
	{
		register int32_t n;
		bool rc=false;
	 ...
	 */
	extern size_t brisc_heap_sizeof(void* pointer)
	{
		size_t rc=0;
		if ( pointer )
		{
	...
	*/
	extern void* brisc_heap_malloc(size_t size)
	{
		void* pointer = NULL;
		if ( size > 0 )
		{
		...
		#if defined(BRISC_CLEAR_HEAP_MALLOC)
			if ( pointer )
	...
	*/
	extern void* brisc_heap_realloc(void* pointer, size_t size)
	{
				...
				if (blocks > used)
				{
					if (!extend(HEAP_STATE(heap_num),block,used,blocks-used))         /* attempt to extend existing block */
					{
						...
						else
						{
							 ...
							 */
							void* pTarget;
							if ( (pTarget = brisc_heap_malloc(size)) != NULL )
							{
		...
		{
			brisc_heap_free(pointer);
			pointer=NULL;
		}
		else if (pointer == NULL )
		{
			pointer = brisc_heap_malloc(size);
		}
		return pointer;
	}
	...
	extern void* brisc_heap_calloc(size_t nmemb, size_t size)
	{
		...
		void* pointer = brisc_heap_malloc(bytes);
	    if ( pointer )
	    {
	...
	** tion is performed.
	*/
	extern void brisc_heap_free(void* pointer)
	{
		int32_t block=(-1);
		int32_t used;
		BRISC_MALLOC_LOCK();
...
#else /* BRISC_NULL_HEAP */


	void* brisc_heap_malloc(size_t sz)
	{
		return NULL;
	}

	void* brisc_heap_realloc(void* p, size_t sz)
	{
		return NULL;
	}

	void* brisc_heap_calloc(size_t nmemb, size_t size)
	{
		return NULL;
	}

	void  brisc_heap_free(void* p)
	{
	...
	int32_t brisc_heap_block_size(void)
	{
		return 0;
	}

	int32_t brisc_heap_blocks_allocated()
	{
		return 0;
	}

	int32_t brisc_heap_bytes_used()
	{
		return 0;
	}

	int32_t brisc_heap_bytes_free()
	{
		return 0;
	}

	size_t  brisc_heap_sizeof(void* p)
	{
		return 0;
	}
	...
	{ 
		return brisc_heap_malloc(sz); 
	}

	extern void* realloc(void* p, size_t sz)         
	{ 
		return brisc_heap_realloc(p, sz); 
	}

	extern void* calloc(size_t nmemb, size_t size)   
	{ 
		return brisc_heap_calloc(nmemb, size); 
	}
